 ...
 * Example file
 *
 * Authenticating the end-user at the highest Quality of Registration (QoR) available
 *
...
$configuration = array(
    ...
    'redirectURL' => 'https://example.com/example3.php', // replace this with your redirect URL registered with SwissID
...
/**
 * Store the first Quality of Registration (QoR) to request,
 * if not already set, in the session
 */
if (!isset($_SESSION['requestedQoR'])) {
    $_SESSION['requestedQoR'] = 2;
}
...
try {
    ...
    $claims = null;
    $qoa = null;
    $qor = 'qor'.$_SESSION['requestedQoR']; // Request the required Quality of Registration (QoR)
    ...
    if ($swissIDConnector->hasError()) {
        ...
        switch ($error['error']) {
            ...
            case 'unmet_authentication_requirements':
                /**
                 * Lower the requested QoR by one and redirect the end-user to give it another try
                 */
                --$_SESSION['requestedQoR'];
        ...
        echo 'Hi there '.$givenName.' '.$familyName.', after you initiated this process on '.date('Y-m-d @ h:i:s', $startDatetime).' (FYI, now it is '.date('h:i:s', time()).'), we figured out that your Quality of Registration is '.$qor;
